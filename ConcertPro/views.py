...
def concerts_agenda(liste_concerts,heures=HEURES1,pas=PAS1):
    ...
    for concert in liste_concerts:
        ...
        if datetime.timedelta(days=-(today.weekday()+1))<concert["date_debut"]-today<datetime.timedelta(days=7-(today.weekday()+1)):
            for h in heures:
                fin_horaire = h+pas
                if fin_horaire > 23:
